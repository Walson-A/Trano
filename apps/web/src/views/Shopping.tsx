...
export function Shopping() {
  ...
  return (
    <div className="max-w-3xl mx-auto pb-24 md:pb-4">
      ...
      {/* Ajout d'article : compact au repos, les options n'apparaissent
          que pendant la saisie pour laisser la liste respirer */}
      <div className="bg-white dark:bg-zinc-900/60 border border-zinc-200 dark:border-white/5 rounded-2xl p-2.5 mb-6">
        <div className="flex gap-2">
          <input
            ...
            onBlur={() => { if (!title.trim()) setComposerOpen(false); }}
            onKeyDown={(e) => e.key === 'Enter' && handleAdd()}
            placeholder="Ajouter un article…"
            className="flex-1 min-w-0 bg-zinc-100 dark:bg-white/5 border border-transparent focus:border-zinc-300 dark:focus:border-white/20 rounded-xl px-4 py-2.5 text-base text-zinc-900 dark:text-zinc-100 placeholder-zinc-400 dark:placeholder-zinc-600 outline-none transition-colors"
          />
          <button
            onClick={handleAdd}
            disabled={!title.trim() || adding}
            className="w-11 h-11 shrink-0 rounded-xl bg-zinc-900 dark:bg-zinc-100 text-zinc-100 dark:text-zinc-900 flex items-center justify-center hover:opacity-90 active:scale-95 transition-all disabled:opacity-30"
            aria-label="Ajouter"
          >
          ...
          {composerOpen && (
            ...
            >
              <div className="flex flex-wrap items-center gap-1.5 pt-2.5">
                ...
                <div className="w-px h-5 bg-zinc-200 dark:bg-white/10 mx-0.5 hidden sm:block" />
                ...
                <select
                  ...
                  className={cn(
                    'px-2.5 py-1.5 rounded-lg text-xs font-medium outline-none cursor-pointer transition-all border-none',
                    recurrenceDays
                      ? 'bg-zinc-900 dark:bg-zinc-100 text-zinc-100 dark:text-zinc-900'
