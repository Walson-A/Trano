...
import { getHouseSnapshot, listControllableDevices, controlDevice, haConfigured } from '../lib/ha.ts';
...
export function houseRoutes(app: FastifyInstance): void {
  app.get<{ Querystring: { profile?: string } }>('/api/house', async (req, reply) => {
    ...
    const favorites = (wanted ?? []).map((id) => byId.get(id)).filter((d): d is Device => Boolean(d));

    return {
      energie: snapshot.energie ?? null,
      meteo: snapshot.meteo ?? null,
      favoris: favorites,
      allumes: devices.filter((d) => ON_STATES.has(d.etat)),
